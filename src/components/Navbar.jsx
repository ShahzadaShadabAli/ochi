...
function Navbar() {
  ...
  return (
    <nav className={`w-full z-[999] fixed px-20 py-8 flex justify-between items-center ${isVisible ? 'slide-down' : 'slide-up'}
    ${scroll > 807 ? "bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg border-none p-6" : ""} `}>
      <div className="logo">
      <svg
        width="72"
        height="30"
      ...
      <div className="links flex gap-10">
        {["Services", "Our Work", "About Us", "Insights", "Contact"].map((item, index) =>(
            <a key={index} href="" className={`text-lg capitalize font-['neue'] font-light ${index === 4 ? "ml-32" : ''}`}>{item}</a>
        ))}
      </div>
    </nav>
  );
